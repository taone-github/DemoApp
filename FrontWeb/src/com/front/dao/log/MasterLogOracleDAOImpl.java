 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class MasterLogOracleDAOImpl extends ConnectionService implements MasterLogDAO {
	private final static transient Logger logger = Logger.getLogger(MasterLogOracleDAOImpl.class);	
	...
	public Vector searchAccessLog(String username, String accessType, Date fromDate, Date toDate, int startRow, int endRow) throws MasterLogException {
		...
		try {
			...
			sql.append(" SELECT ROWNUM AS ROW_NUM,T1.* FROM ( ");
			sql.append(" SELECT REF_ID,APP_CODE,PROC_DT,PROC_USER_ID, ");
			sql.append(" LOG_ACTION,LOG_MODULE, ");
			sql.append(" OLD_DAT,NEW_DAT,IP_ADDRESS ");
	...
	public HashMap loadActivityTypeList(String logType) throws MasterLogException {
		...
		try {
			conn = getConnection(JDBCServiceLocator.MASTER_DB);
			StringBuffer sql = new StringBuffer("");
			sql.append(" SELECT DISTINCT \"ID\",LIST_CODE,LIST_DESC FROM M90_SLLST WHERE LIST_TYPE = ? ORDER BY \"ID\" ");
			String dSql = String.valueOf(sql);
			logger.debug("SQL = " + dSql);
			ps = conn.prepareStatement(dSql);
	...
	public HashMap loadActionTypeList(String logType) throws MasterLogException {
		...
		try {
			conn = getConnection(JDBCServiceLocator.MASTER_DB);
			StringBuffer sql = new StringBuffer("");
			sql.append(" SELECT DISTINCT LOG_ACTION FROM M_ENTITY_LOG WHERE LOG_TYPE = ? ORDER BY LOG_ACTION ");
			String dSql = String.valueOf(sql);
			logger.debug("SQL = " + dSql);
			ps = conn.prepareStatement(dSql);
			ps.setString(1, logType);
	...
	public HashMap loadAccessTypeList() throws MasterLogException {
		...
		try {
			conn = getConnection(JDBCServiceLocator.MASTER_DB);
			StringBuffer sql = new StringBuffer("");
			sql.append(" SELECT DISTINCT LOG_MODULE FROM M_ENTITY_LOG WHERE LOG_TYPE='2' ORDER BY LOG_MODULE ");
			String dSql = String.valueOf(sql);
			logger.debug("SQL = " + dSql);
			ps = conn.prepareStatement(dSql);
			rs = ps.executeQuery();
