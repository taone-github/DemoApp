		...
		%>		
				<!-- Menu lv1 -->
	              <a id='<%=topLevelMenuId %>' href="#">
	              	<%-- <img class="icon-menu" src="<%=topLevelMenu.getIcon() %>"></img> --%>
	              	<span class='wordwrap wordwrap-text-lv1'><%=topLevelMenu.getMenuName() %></span><i class="fa fa-angle-left pull-right"></i>
	              </a> 
	            	...
	            	%>
	            			 <!-- Menu lv2 -->
								<a id='<%=menuId %>' href="#">
									...
									<span class="wordwrap-lv2"><%=subMenu.getMenuName() %></span><i class="fa fa-angle-left pull-right"></i>
								</a>
					            ...
					            if(subMenus2 != null && !subMenus2.isEmpty()) {
									...
									%>
										 <!-- Menu lv3 -->
										 <a id='<%=menuId2 %>' href="#">
										 	<%-- <img class="icon-menu-lv3" src="<%=subMenu2.getIcon() %>"></img> --%>
										 	<span class='wordwrap-lv3'><%=subMenu2.getMenuName() %></span><i class="fa fa-angle-left pull-right"></i>
									 	 </a>
										 <ul class="treeview-menu addmenulv3-color">
									<%
											...
											if(subMenus3 != null && !subMenus3.isEmpty()) {
												...
												for(MenuM subMenu3: subMenus3){
													...
													System.out.println("### arom >>"+subMenu3.getMenuName());
												
									            %>
									            <!-- Menu lv4 -->
									            	<li>
									            		<a id='<%= menuId3%>' href='<%=url%>' target='mainPage'>
									            			<i class="fa fa-angle-right" onclick=showLoader()></i>
									            			<span class='wordwrap-lv3'><%=subMenu3.getMenuName() %></span>
									            		</a>
								            		</li>
								    	        <%}
											} %>
						...
						%>
							  	<a id='<%= menuId%>' href='<%=url%>' target='mainPage' onclick=showLoader()>
							  		<img class="icon-menu" src="<%=subMenu.getIcon() %>"></img>
	        ...
	        %>
	        		<a id='<%=topLevelMenuId %>' href='<%=url%>' target='mainPage'  onclick=showLoader()>
	        			<%-- <img class="icon-menu" src="<%=topLevelMenu.getIcon() %>"></img>  --%>
		              	<span><div class='wordwrap wordwrap-text-lv1'><%=topLevelMenu.getMenuName() %></div></span>
		            </a>
		    <%} %>
		    </li>
		<%  }
