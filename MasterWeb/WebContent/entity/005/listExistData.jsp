...
%>     
						<th width = "4%" align="center" height="19" class="tableHeader">&nbsp;</th>
<% 
	} else {
		totalWidth = 96/vShowColumns.size();
	}
%>
						<th width = "4%"  align="center" height="19" class="tableHeader"><%=MessageResourceUtil.getTextDescription(request, "WORDING.ITEM_NO") %></th>
<%

	for (int i= 0;i < vShowColumns.size();i++) {
		DuplicationFuncM  prmDuplicationFuncM = (DuplicationFuncM)vShowColumns.get(i);	
%>      
						<th width ="<%=totalWidth%>%" align="center" class="tableHeader"><%=MasterUtil.displayFieldName(prmDuplicationFuncM,request)%> </th>
<% 
	} 
%>
					</tr>
