...
%>						
						<input type="button" name="selectAll" value="+" class="button_style_1" onclick="addRecord('All','<%= form.getMainModuleID()%>','<%=MasterForm.getVShowSearchRecord().size()%>')" >
<% 
	}
%>						
						</th>       
...
%>      
						<th align="center" class="tableHeader <%=moduleM.getModuleID()%>_<%=moduleM.getFieldID()%>_header"><%=MasterUtil.displayFieldName(moduleM,request)%> </th>
<%
	}
%>
					</tr>
...
%>	
							<span id = "select_<%=i%>">
								<input type="button" name="select" value="+" class="button_style_1" onclick="addRecord('<%=i%>','<%= form.getMainModuleID()%>','<%=MasterForm.getVShowSearchRecord().size()%>')" >
							</span>
						</td>
<% 
	}
