...
<%if(vStoreActions.size()>0  && (!MasterForm.isUnEditManyFlag())){ %>
					<th width = "3%" height="19" class="tableHeader"></th>
<%} %>
					<th width = "4%" height="19" class="tableHeader"><%=MessageResourceUtil.getTextDescription(request, "WORDING.ITEM_NO") %></th>       
<%
for (int i= 0;i < vShowColumns.size();i++) {
	ModuleFieldM  moduleM = (ModuleFieldM)vShowColumns.get(i);

%>      
					<th width ="<%=totalWidth%>%" class="tableHeader" ><%=MasterUtil.displayFieldName(moduleM,request)%></th>
<% 
 }
%>
<%if(vStoreActions.size()>0 && (!MasterForm.isUnEditManyFlag())){ %>
