...
<%@page import="com.master.model.ModuleActionM"%><jsp:include flush="true" page="../manyRelationEventScript.jsp"/>
...
<%
	...
	String schemaName = MasterUtil.getDynamicSchema(request);
	MasterDAO dao = MasterDAOFactory.getMasterDAO(entityID, schemaName);
	HashMap hKeys = dao.getFieldKeyInfo(module);
	Vector vKeys = new Vector(hKeys.keySet());
%>
...
<tr>
<td valign="bottom" align="right" width="24"><img src="./theme/005/images/line_top_left_2.gif" width="24" height="25"></td>
<td width="100%"><img src="./theme/005/images/line_top_2.gif" width="100%" height="25"></td>
<td valign="bottom" width="24"><img src="./theme/005/images/line_top_right_2.gif" width="24" height="25"></td>
</tr>
<tr>
<td background="./theme/005/images/line_left_2.gif"></td>
<td-->
...
%>				
				...
				<td class="td_OneRelation" >
					<div class="componentDiv" id="<%=filterFieldM.getModuleID()%>_<%=filterFieldM.getFieldID()%>_filter" ><%=tagShow%></div>
				</td>

<% 
		if (((u%2) == 1) || ((u+1)==vFilters.size()) ) {		
%>				
				</tr>
<% 
		}
			...
			<table border="0" cellspacing=1 cellpadding=1 width="100%" >
			    <tr >
			      <td class="text" align=center>
						...
						<jsp:param name="action" value="FILTER_MANY"/>
						</jsp:include>			 
...
<% 	
	//Sam change viewModeFlag not need for paging
	//27/12/2011
	...
	if (MasterForm.getModuleM().isPagingFlag() && 
		MasterConstant.PROCESS_MODE_UPDATE.equals(form.getCurrentMode())
		&& MasterForm.getAllSearchResultData() > 0) { 
%>
...
<%if(vStoreActions.size()>0  && (!MasterForm.isUnEditManyFlag())){ 
			...
			String checked ="";
			if(MasterForm.getModuleM().isPagingFlag()){
				if(CheckBoxUtil.isAllChecked(module,request)){
					checked = "checked";
				}
			}

%>
					<th width = "3%" height="35" class="tableHeader">
						<!--<input type=checkbox name=deleteAll onclick=<%=MasterForm.getModuleM().getModuleID()%>checkAllList() <%=checked%>> -->
						    <input type=checkbox name=deleteAll onclick=<%=MasterForm.getModuleM().getModuleID()%>checkAllList()> 
					</th>
<%} %>					
					<th width = "4%" height="35" class="tableHeader"><%=MessageResourceUtil.getTextDescription(request, "WORDING.ITEM_NO") %></th>       
<%
for (int i= 0;i < vShowColumns.size();i++) {
	ModuleFieldM  moduleM = (ModuleFieldM)vShowColumns.get(i);

%>      
					<th class="tableHeader <%=moduleM.getModuleID()%>_<%=moduleM.getFieldID()%>_header" >
<%
			if (MasterForm.getModuleM().isOrderingFlag() && MasterForm.getModuleM().isViewModeFlag() && 
					MasterConstant.PROCESS_MODE_UPDATE.equals(form.getCurrentMode())) {
%>
...
<%if(vStoreActions.size()>0 && ((!MasterForm.isUnEditManyFlag()) && (!hideEdit)) && (!unEditInLine) ){ %>
					<th class="tableHeader"><div style="display: block; width: 30px" ></div></th>
<%} %>
				</tr>
				</thead>
				<tbody>
...
<%if(vStoreActions.size()==0){ %>
				<tr class="ROW" onmouseover="this.className='ROW_HOVER'" onmouseout="this.className='ROW'" height="35">
...
<%
			for (int i= 0;i < vStoreActions.size();i++) {
				String keyForSearch = Integer.toString(i);
				...
				if (!MasterForm.isUnEditManyFlag()) {
					classOver  = "onmouseover=\"this.className='ROW_HOVER'\"";
				}
%>
				<tr height="35" class="<%=((i%2)==0)? "ROW" : "ROW_ALT" %>" <%=classOver%>  onmouseout="<%=((i%2)==0)? "this.className='ROW'" : "this.className='ROW_ALT'"%>">
<% if (!MasterForm.isUnEditManyFlag()) {
		//Sam fix checkbox add script
		//26/12/2011
		String checkBoxPageing = "";
		if(MasterForm.getModuleM().isPagingFlag()){
			
			//old value of checkbox is i
			//use key as value
			keyForSearch = "";
			for (int j=0; j < vKeysAction.size();j++) {
				...
				HashMap hTables = (HashMap)hStoreActions.get(key);	
		  		...
		  		if(hKeyForSearch == null) {
		  			hKeyForSearch = CheckBoxUtil.getTempKeyForSearch(vKeys, request);
		  		}
			...
			if(CheckBoxUtil.isChecked(module,keyForSearch,request)){
				checked = "checked";
			}
			
			// ----------- comment by worrachet 2012-05-09 for check header checkbox when all item is checked -------------
			//checkBoxPageing = "onclick=\"document.getElementsByName('deleteAll')[0].checked=false;changeCheckBoxMany(this,'"+module+"')\" "+checked;
			checkBoxPageing = "onclick=\"$('#"+module+"MG').find('[name=deleteAll]').attr('checked', false);changeCheckBoxMany(this,'"+module+"')\" "+checked;
		}
%>
					<!-- <td width ="3%" align="center"><input type = "checkbox" name = "<%=module%>deleteRow" value ="<%=keyForSearch%>" <%=checkBoxPageing%>  <%=(vStoreActions.size() > 0) ? "" : "disabled" %> > </td>-->
					     <td width ="3%" align="center"><input type = "checkbox" name = "<%=module%>deleteRow" value ="<%=keyForSearch%>"  onclick="<%=module%>checkList();"                     <%=(vStoreActions.size() > 0) ? "" : "disabled" %> > </td>
<% } %>					
...
<!--					<td width ="4%" height="19" align ="center" ><%=(i+1)%></td>-->
						<td width ="4%" height="19" align ="center" ><%=lineNumber%></td>
<% 					
				for (int j=0; j < vKeysAction.size();j++) {
					String key = (String)vKeysAction.get(j);
					HashMap hTables = (HashMap)hStoreActions.get(key);	
...
<!-- /td>
<td background="./theme/005/images/line_right_2.gif" ></td>
</tr>
<tr>
	<td valign="bottom" align="right" width="24"><img src="./theme/005/images/line_bot_left_2.gif" width="24" height="25"></td>
	<td width="100%"><img src="./theme/005/images/line_bot_2.gif" width="100%" height="25"></td>
	<td valign="bottom" width="24"><img src="./theme/005/images/line_bot_right_2.gif" width="24" height="25"></td>
</tr>
</table-->
