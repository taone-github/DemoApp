...
%>
<table align = "right" width="100%"  cellpadding="2" cellspacing="0" border="0">
	<tr>
		<td width="100%">&nbsp;</td>
		<td align="right" valign="top">
			...
			<jsp:param name="action" value="SUB_SEARCH"></jsp:param>
			</jsp:include>
		</TD>
	</tr>
</table>
<br>
<TABLE width="100%" border="0" cellpadding="0" cellspacing="0" align="center" border-color:#666666>
	<tr><td>&nbsp;</td></tr>
	<tr height="19">
		<td colspan="4" valign="top">
			<div id="contentResult">
				<table width="100%" border="0"  cellpadding="0" cellspacing="0" align="center" class="contentResult">
					<tr class="title" >
...
/* if there are no search result, height of div result will be as two row */
							...
							<td class="<%=moduleFieldM.getModuleID()%>_<%=moduleFieldM.getFieldID()%>_col">
								<%if(MasterConstant.TEXTBOX.equalsIgnoreCase(moduleFieldM.getObjType()) 
									...
									<div id="<%=labelNamed %>" class="showElement">
										<a href="javascript:void(0)" onclick="swapElement('<%=htmlComponentNamed %>','<%=labelNamed %>','<%=i %>', '<%=moduleFieldM.getObjType() %>', '<%=moduleFieldM.getModuleID() %>', '<%=moduleFieldM.getFieldID() %>')" ><%=strSearchResult%></a>&nbsp;<%=buttonPopup%>
									</div>
							...
							<%/* case -> dont have value in this td <-> display in the result dataTable */%>
			  				<td class="<%=moduleFieldM.getModuleID()%>_<%=moduleFieldM.getFieldID()%>_col">
			  					<%if(MasterConstant.TEXTBOX.equalsIgnoreCase(moduleFieldM.getObjType()) 
			  							...
			  							|| MasterConstant.POPUP.equalsIgnoreCase(moduleFieldM.getObjType())){ %>
				  					<div id="<%=labelNamed %>" class="showElement">
										<a href="javascript:void(0)" onclick="swapElement('<%=htmlComponentNamed %>','<%=labelNamed %>','<%=i %>', '<%=moduleFieldM.getObjType() %>', '<%=moduleFieldM.getModuleID() %>', '<%=moduleFieldM.getFieldID() %>')" >&nbsp;</a>
									</div>
									<div id="<%=htmlComponentNamed %>" class="hideElement">
