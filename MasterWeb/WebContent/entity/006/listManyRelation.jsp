...
%>      
					<th width ="<%=totalWidth%>%" class="TableHeaderList" ><%=MasterUtil.displayFieldName(moduleM,request)%></th>
<% 
 }
%>
<%if(vStoreActions.size()>0 && (!MasterForm.isUnEditManyFlag())){ %>
							...
							<td width="<%=totalWidth%>%">
								<%if(!MasterConstant.CHECKBOX.equalsIgnoreCase(moduleFieldM.getObjType()) 
									...
									<div id="<%=labelNamed %>" class="showElement">
										<a href="javascript:void(0)" onclick="drawElementAjax('<%=htmlComponentNamed %>','<%=labelNamed %>','<%=i %>', '<%=moduleFieldM.getObjType() %>', '<%=moduleFieldM.getModuleID() %>', '<%=moduleFieldM.getFieldID() %>')" ><%=MasterUtil.generateDataFormatInManyRow(strSearchResult, moduleFieldM.getMfID(), hFieldProperty)%></a>&nbsp;<%=buttonPopup%>
										<%if(MasterConstant.POPUP.equalsIgnoreCase(moduleFieldM.getObjType())){ %>
							...
							<%/* case -> dont have value in this td <-> display in the result dataTable */%>
			  				<td width="<%=totalWidth%>%">
			  					<%if(!MasterConstant.CHECKBOX.equalsIgnoreCase(moduleFieldM.getObjType()) 
										...
										&& !MasterConstant.NOT_OBJECT.equalsIgnoreCase(moduleFieldM.getObjType())){ %>
				  					<div id="<%=labelNamed %>" class="showElement">
				  						<a href="javascript:void(0)" onclick="drawElementAjax('<%=htmlComponentNamed %>','<%=labelNamed %>','<%=i %>', '<%=moduleFieldM.getObjType() %>', '<%=moduleFieldM.getModuleID() %>', '<%=moduleFieldM.getFieldID() %>')" >&nbsp;</a>
				  						<%if(MasterConstant.POPUP.equalsIgnoreCase(moduleFieldM.getObjType())){ %>
											<input name="<%=MasterConstant.NAMING.POP_UP_MF_ID %>" type="hidden" value="<%=moduleFieldM.getMfID() %>"/>
										<%} %>
									</div>
