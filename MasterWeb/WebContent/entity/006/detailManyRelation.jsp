...
%>
				<tr bgcolor="white">
					...
					for (int i= 0;i < vShowColumns.size();i++) {
						...
						for (int u =0; u < vFilters.size();u++) {
							...
							if(MasterUtil.displayFieldName(moduleM,request).equals(MasterUtil.displayFieldName(filterFieldM,request))){
								filterID = filterFieldM.getModuleID()+"_"+filterFieldM.getFieldID()+"_filter";
								tagShow = MasterUtil.displayFilterTag(request, entityID, filterFieldM.getFieldID(), MasterForm);
								break;
							}else{
								filterID = "";
								tagShow = "";
				...
				%>      
					<td width ="<%=totalWidth%>%">
					<div class="componentDiv" id="<%=filterID %>" style="width:100%" align="center"><%=tagShow%></div></td>
<% 
						
 					}
%>
<%if(vStoreActions.size()>0 && (!MasterForm.isUnEditManyFlag()) && (!hideEdit)){ %>
					<td><div style="display: block; width: 30px" ></div></td>
<%} %>
...
%>											
										<div  id ="orderASCDivID_<%=i %>" class="orderASCDiv" onclick="<%=module%>ManyOrder('orderASCDivID','<%=i %>','<%=moduleM.getFieldID() %>', 'ASC');"></div>
										<div  id ="orderDESCDivID_<%=i %>" class="orderDESCDiv" onclick="<%=module%>ManyOrder('orderDESCDivID','<%=i %>','<%=moduleM.getFieldID() %>', 'DESC');"></div>
<% 
				}
%>												
									</td>
...
jQuery("#<%=MasterForm.getModuleM().getModuleID()%>MG").find("input:text").keypress(keyEnter);

function keyEnter() {
	if(event.keyCode == 13){ 
		<%=MasterForm.getModuleM().getModuleID()%>FilterData();
	}		
}
