...
<%
	...
	boolean viewMode = entityForm.getEntityM().isViewModeFlag();
	if (!viewMode) {
		viewMode = MasterForm.getModuleM().isViewModeFlag();
	}
...
<%@page import="com.master.form.EntityFormHandler"%><table class="tableForm" align="center" width="90%" cellpadding="0" cellspacing="0" border="0">
...
<% 
	if (!viewMode) {
		viewMode = prmModuleFieldM.isViewFlag();
	}
	String align = "";
	if (viewMode && MasterConstant.PROCESS_MODE_UPDATE.equals(entityForm.getCurrentMode()) 
		&& (prmModuleFieldM.getAlignment() !=null && ! "".equals(prmModuleFieldM.getAlignment())) ) {
		align = "style=\"text-align: "+prmModuleFieldM.getAlignment()+"\"";
	}
 
%>				
				<td class="td_OneRelation <%=MasterForm.getModuleM().getModuleID()%>_<%=prmModuleFieldM.getFieldID()%>_input" <%=lastRowColSpan %>  >
					<div class="componentDiv"  id="<%=htmlComponentNamed %>" <%=align%>><%=tagShow%></div>
				</td>
<%
...
$(function(){
	$('input[type=text] , textarea').focus(function(e){		
		if(this.readOnly  == false){
			$(this).css('background-color','#FFDECE');
		}
	});	
