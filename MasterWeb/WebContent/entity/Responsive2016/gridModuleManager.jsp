...
<%
	...
	* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
	...
	if("Y".equals(moduleForm.getModuleM().getIsCollapseMD())){
		isToggleMD = " onclick=\"toggle"+moduleForm.getModuleM().getModuleID()+"()\" ";
		if(null!=request.getSession().getAttribute(moduleForm.getModuleM().getModuleID()+"CollapseMD")){
			collapseModule = (String)request.getSession().getAttribute(moduleForm.getModuleM().getModuleID()+"CollapseMD");
		}else if("Y".equals(moduleForm.getModuleM().getDfCollapseMD())){
			collapseModule = "Y";
		}
			
	}
%>
<div id="<%=moduleForm.getModuleM().getModuleID()%>Error"></div>

		<div id="<%=htmlModuleDivID %>" class="boxbold3" <%=(moduleForm.getModuleM().isAclFlag() && AccessControlM.ACCESS_MODE.NO_ACCESS.equals(moduleForm.getModuleM().getAclMode()) ? "style='display:none;'" : "") %>>
			<div id ='<%=moduleM.getModuleID()%>MG'  >
			<div class="panel-subheading02" style="width:600px;" <%=isToggleMD%>>
			<% if("Y".equals(moduleForm.getModuleM().getIsCollapseMD())){ %>
				<div id='<%=moduleForm.getModuleM().getModuleID()%>-arrow' style='float:left;font-size:1.5em;margin-top:0.1em;padding-right:0.15em' class='fa fa-minus'></div>
			...
			<%	if("Y".equals(collapseModule)){  %>
				toggle<%=moduleForm.getModuleM().getModuleID()%>();
			<% } %>
			});
</script>
<%
