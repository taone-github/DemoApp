...
<div id="module-many-button" style="width:98%" class="module-many-button">
...
%>
							...
							<td>
							<div class="text-header2"><%=MasterUtil.displayFieldName(moduleM,request)%>
...
<!-- 									</div> -->
							...
							%>
							<td  align="<%=alignment%>" class="datatable-many <%=moduleFieldM.getModuleID()%>_<%=moduleFieldM.getFieldID()%>_col" colspan="<%=colspan%>"<%=linkTD%>>
...
<!-- 									<div>TESTT</div> -->
									<div id="<%=labelNamed %>" class="showElement <%=classEditable%>" <%=linkEditInLine%>  style="float: <%=alignment%>;text-align: right">										
...
<!-- 							    <div>TESTT2</div> -->
									...
									<%}
									//---first row Mannydetail for Mobile byp
									
									 if (!MasterForm.isUnEditManyFlag() && MasterUtil.displayFieldName(moduleM,request).equals("ลำดับที่")) {%>					
									<td class="text-center" width ="3%"><input type = "checkbox" name = "<%=module%>deleteRow" value ="<%=i%>" data-size="large"></td>
									<% if (((!MasterForm.isUnEditManyFlag() && (!hideEdit)) && (!unEditInLine)) || "Y".equals(isAlwaysShowEditBtn)) {%>	 					
									<td class="<%=MasterForm.getModuleM().getModuleID()%>_edit_button_col" width ="3%">
									...
									<% } %><%
									//---End first row Mannydetail for Mobile byp
...
%>
						 ...
						 <tr class="<%=highlightClassName %> <%=((i%2)==0)? "gumaygrey2" : "gumaygrey2" %>" onmouseover="this.className='<%=highlightClassName%> gumaygrey2'" onmouseout="<%=((i%2)==0)? "this.className='"+highlightClassName+" gumaygrey2'" : "this.className='"+highlightClassName+" gumaygrey2'"%>">
			  				<td align="<%=alignment%>" class="datatable-many <%=moduleFieldM.getModuleID()%>_<%=moduleFieldM.getFieldID()%>_col" <%=linkTD%> >
			  					<div class="text-header2"><%=MasterUtil.displayFieldName(moduleM,request)%></div>
			  					<%=nextEntityButton %>
			  					<%if(!MasterConstant.CHECKBOX.equalsIgnoreCase(moduleFieldM.getObjType()) 
			  							&& !MasterConstant.MULTISELLIST.equalsIgnoreCase(moduleFieldM.getObjType()) 
										&& !MasterConstant.ATTACHMENTLISTBOX.equalsIgnoreCase(moduleFieldM.getObjType()) 
...
%>					
<% if (((!MasterForm.isUnEditManyFlag() && (!hideEdit)) && (!unEditInLine)) || "Y".equals(isAlwaysShowEditBtn)) {%>	 					
<%-- 					<td class="<%=MasterForm.getModuleM().getModuleID()%>_edit_button_col"> --%>
<%-- 						<div id="<%=MasterForm.getModuleM().getModuleID() + "_BUTTON_EDIT_"+i%>" class="showElement"> --%>
		<%if(moreChildFlag){
			if(moreChildEditBtn || "Y".equals(isAlwaysShowEditBtn)){
		%>
