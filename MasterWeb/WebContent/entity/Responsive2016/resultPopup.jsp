				...
				<table width="100%" cellpadding="1" cellspacing="1" align="center">
					...
					for (int j=0;j < vShowSearchRecs.size();j++) {
						...
						for (int k=0;k < vPopColumns.size();k++) {	
							...
							%>
								<td align="<%=popupColNameM.getAlignment()%>" class="bu2 <%=MasterForm.getModuleM().getModuleID()%>_<%=popupColNameM.getMfID()%>_col"><a href="Javascript:selectitem<%=j%>();"><%=strSearchResult%></a></td>
							<%	
							} else {
							%>
								<td align="<%=popupColNameM.getAlignment()%>" class="bu2 <%=MasterForm.getModuleM().getModuleID()%>_<%=popupColNameM.getMfID()%>_col">&nbsp;</td>
							<%	
							}
						}
