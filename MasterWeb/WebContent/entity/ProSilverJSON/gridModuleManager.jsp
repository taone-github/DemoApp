...
if(!"".equalsIgnoreCase(currentTab) && null != currentTab){
	...
	//FT020 logic display composite module
	com.avalant.feature.FT020_CompositeModule comModChecker = new com.avalant.feature.FT020_CompositeModule();
	...
	for(int i = 0;i < vTabModuleMs.size();i++) {
		 ...
		 String comId = com.avalant.feature.FT020_CompositeModule.getCompositeId(entityID,moduleM.getModuleID());
		 if(null!=comId){
		 	//this module is composite
		 	if(!comModChecker.isCompositeModuleDisplayed(comId)){//this composite need to display
		 		comModChecker.setDisplayedComId(comId);
%>
		<div id="<%=htmlModuleDivID %>" class="boxbold3">
			<!-- Display composite module -->
			<div id ='<%=comId%>MG'>
				<jsp:include flush="true" page="compositeModule.jsp">
	 	 		<jsp:param name="GRID_MODULE" value="Y"></jsp:param>
	 	 		<jsp:param name="comId" value="<%=comId%>"></jsp:param>
	 	 		</jsp:include>
	 	 	</div>
		</div>
<%
