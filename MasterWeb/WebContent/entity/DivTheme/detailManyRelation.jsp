...
%>	
<div id="module-many-button" style="display: none">
...
%>
	...
	<div id="<%=module%>Many" align="center" class="tablearea" style="width: <%=moduleWidth%>%;">  
...
<% if ((!MasterForm.isUnEditManyFlag() && (!hideEdit)) && (!unEditInLine)) {%>	 					
		...
		<%if(moreChildFlag){%>
							<div class="addicon" name="<%=module%><%=module%>ButtonRowET" onClick="saveEntityForUpdateChildEntity('<%=nextEntityID%>','<%=nextTabID%>','<%=keyForSearch%>')" ></div>
			<%}else{%>
							<div class="addicon" name="<%=module%>ButtonRow" onClick="loadUpdateMany('<%=module%>','<%=i%>')" ></div>
		<%}%>
						</div>					
					</td>
<% } %>					
