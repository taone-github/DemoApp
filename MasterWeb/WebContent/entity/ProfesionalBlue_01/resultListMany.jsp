...
<% 
	...
	int volumePerpage = MasterForm.getVolumePerPage();
%>
...
<input type ="hidden" name = "volumePerPage" value = "<%=volumePerpage%>">
...
<%
	...
	System.out.println(" ----------- Sam check result ListMany ------------ : " + listOnTab);
...
<% 
if(mainForm.getModuleM().isPagingFlag()){
	if (vShowSearchRecs.size() > 0) {
...
<% 	
	...
	for(int i =0;i< v.size();i++) {
		if (volumePerpage == Integer.parseInt((String)LoadXML.getLoadXML(MasterConstant.EAF_MASTER_NAME).getItemPerPageMap().get(Integer.toString(i+1)))) {
%>     
...
<% 
	int allPage =  MasterForm.getAllSearchResultData() / volumePerpage;
	if (allPage*volumePerpage  < MasterForm.getAllSearchResultData()) {
		allPage++;
	}
