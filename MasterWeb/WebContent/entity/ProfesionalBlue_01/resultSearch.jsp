	...
	<tr>
		<td colspan="4" valign="top">
			<div align="center"  id ="manyRowDivID" class="manyRowDiv">
				<table width="100%" cellpadding="1" cellspacing="0" align="center" class="gumayframe3" bgColor="#ffffff">
					<tbody>
						<tr height="19" class="gumayframe2">     
<%      
	int totalWidth = 0;
...
%>     
							...
							<th width = "4%" align="center" height="19" class="TableHeaderList"><input type = "<%=objType%>" name = "deleteAll" value ="deleteAllValue" onclick="checkAllList()" <%=checked%> ></th>
<% 
	} else {
		totalWidth = 96/vShowColumns.size();
	}
%>
							<td width = "4%"  align="center" height="19" class="TableHeaderList"><%=MessageResourceUtil.getTextDescription(request, "WORDING.ITEM_NO") %></td>       
<%
...
%>      
							<td align="center" class="<%=moduleM.getModuleID()%>_<%=moduleM.getFieldID()%>_header">
...
%>
			<tr class="<%=((i%2)==0)? "gumaygrey2" : "gumaygrey2" %>">
<% 
	}
	if (entityM.isShowCheckBox() || entityM.isShowRadio()) {
		String objType = "checkbox";
...
%>     
							<td  width ="4%"  align="center" height="19" class="bu2"><input type = "<%=objType%>" name = "deleteRow" value ="<%=keyForSearch%>" onclick="document.masterForm.deleteAll.checked=false;changeCheckBox(this)" <%=checked%>></td>
<% 
	}
%>
...
%>		
		<td align="<%=align%>" class="bu2 <%=moduleM.getModuleID()%>_<%=moduleM.getFieldID()%>_col"><div id="<%=moduleM.getFieldID()%>_<%=i%>"><%=tagStr%></div></td>					 
<%			
	}	
%>
						</tr>
